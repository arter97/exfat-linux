...
#define EXFAT_EF_CLUS_MAX	17	/* 512 Byte ~ 32 MByte */
...
static struct exfat_statistics {
	u32 clus_vfat[EXFAT_VF_CLUS_MAX];
	u32 clus_exfat[EXFAT_EF_CLUS_MAX];
	u32 mnt_cnt[EXFAT_MNT_MAX];
	u32 nofat_op[EXFAT_OP_MAX];
	u32 vol_size[EXFAT_VOL_MAX];
} statistics;

static struct kset *exfat_statistics_kset;
				...
				struct kobj_attribute *attr, char *buff)
{
	return snprintf(buff, PAGE_SIZE, "\"FAT12_MNT_I\":\"%u\","
			...
			statistics.mnt_cnt[EXFAT_MNT_FAT12],
			statistics.mnt_cnt[EXFAT_MNT_FAT16],
			statistics.mnt_cnt[EXFAT_MNT_FAT32],
			statistics.mnt_cnt[EXFAT_MNT_EXFAT],
			statistics.mnt_cnt[EXFAT_MNT_RO]);
}
				...
				struct kobj_attribute *attr, char *buff)
{
	return snprintf(buff, PAGE_SIZE, "\"NOFAT_MOUNT_I\":\"%u\","
			...
			"\"NOFAT_TRUNC_I\":\"%u\"\n",
			statistics.nofat_op[EXFAT_OP_EXFAT_MNT],
			statistics.nofat_op[EXFAT_OP_MKDIR],
			statistics.nofat_op[EXFAT_OP_CREATE],
			statistics.nofat_op[EXFAT_OP_READ],
			statistics.nofat_op[EXFAT_OP_WRITE],
			statistics.nofat_op[EXFAT_OP_TRUNC]);
}
				...
				struct kobj_attribute *attr, char *buff)
{
	return snprintf(buff, PAGE_SIZE, "\"VOL_4G_I\":\"%u\","
			...
			statistics.vol_size[EXFAT_VOL_4G],
			statistics.vol_size[EXFAT_VOL_8G],
			statistics.vol_size[EXFAT_VOL_16G],
			statistics.vol_size[EXFAT_VOL_32G],
			statistics.vol_size[EXFAT_VOL_64G],
			statistics.vol_size[EXFAT_VOL_128G],
			statistics.vol_size[EXFAT_VOL_256G],
			statistics.vol_size[EXFAT_VOL_512G],
...
int exfat_statistics_init(struct kset *exfat_kset)
{
	int err;

	exfat_statistics_kset = kset_create_and_add("statistics", NULL, &exfat_kset->kobj);
	if (!exfat_statistics_kset) {
		pr_err("[EXFAT] failed to create exfat statistics kobj\n");
		return -ENOMEM;
	}

	err = sysfs_create_group(&exfat_statistics_kset->kobj, &attr_group_statistics);
	if (err) {
		pr_err("[EXFAT] failed to create exfat statistics attributes\n");
		kset_unregister(exfat_statistics_kset);
		exfat_statistics_kset = NULL;
		return err;
	}
...
void exfat_statistics_uninit(void)
{
	if (exfat_statistics_kset) {
		sysfs_remove_group(&exfat_statistics_kset->kobj, &attr_group_statistics);
		kset_unregister(exfat_statistics_kset);
		exfat_statistics_kset = NULL;
	}
	memset(&statistics, 0, sizeof(struct exfat_statistics));
}

void exfat_statistics_set_mnt(FS_INFO_T *fsi)
{
	if (fsi->vol_type == EXFAT) {
		statistics.mnt_cnt[EXFAT_MNT_EXFAT]++;
		statistics.nofat_op[EXFAT_OP_EXFAT_MNT] = 1;
		if (fsi->sect_per_clus_bits < EXFAT_EF_CLUS_MAX)
			statistics.clus_exfat[fsi->sect_per_clus_bits]++;
		else
			statistics.clus_exfat[EXFAT_EF_CLUS_MAX - 1]++;
		return;
	}
	...
	if (fsi->sect_per_clus_bits < EXFAT_VF_CLUS_MAX)
		statistics.clus_vfat[fsi->sect_per_clus_bits]++;
	else
		statistics.clus_vfat[EXFAT_VF_CLUS_MAX - 1]++;
}

void exfat_statistics_set_mnt_ro(void)
{
	statistics.mnt_cnt[EXFAT_MNT_RO]++;
}

void exfat_statistics_set_mkdir(u8 flags)
{
	if (flags != 0x03)
		return;
	statistics.nofat_op[EXFAT_OP_MKDIR] = 1;
}

void exfat_statistics_set_create(u8 flags)
{
	if (flags != 0x03)
		return;
	statistics.nofat_op[EXFAT_OP_CREATE] = 1;
}
 ...
 */
void exfat_statistics_set_rw(u8 flags, u32 clu_offset, s32 create)
{
	if ((flags == 0x03) && (clu_offset > 1)) {
		if (create)
			statistics.nofat_op[EXFAT_OP_WRITE] = 1;
		else
			statistics.nofat_op[EXFAT_OP_READ] = 1;
	}
}
 ...
 */
void exfat_statistics_set_trunc(u8 flags, CHAIN_T *clu)
{
	if ((flags == 0x03) && (clu->size > 1))
		statistics.nofat_op[EXFAT_OP_TRUNC] = 1;
}

void exfat_statistics_set_vol_size(struct super_block *sb)
{
	u64 vol_size;
